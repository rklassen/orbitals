        ...
        if basis == 'QHO' or approxMethod == 'QHO':
            ...
            for n in range(self.dim):
                QHOBasisApprox.append(
                    FunctionSampler(
                        lambda x: self.QHOBasis(n,x),-15,15,2000
                    )
                )
        ...
        if approxMethod == 'QHO':
            ...
            for i, j in itertools.product(range(self.dim), range(self.dim)):
                HFunc = lambda x: np.conj(
                    ...
                    -1/2 * derivative(
                        lambda xPrime: self.QHOBasis(2, xPrime), x, dx=1e-6, n=2
                    ) + self.V(x) * self.QHOBasis(2, x)
                )
...
class Plotter:
    ...
    def plotWaveFunction3d(
        ...
        def animate(i):
            ...
            ylist = [(imValues, reValues), (np.zeros_like(posValues), probValues)]
...
hs = HilbertSpace(
    dim = 2,
    hamiltonianPotential = lambda x: 1/2*x**2,
    basis = 'QHO',
    approxMethod = None,
)
psi = WaveFunction(hs, coeff = [1,0,1])
#psi = WaveFunction(hs, lambda x: 1 if abs(x) < 5 else 0)
print('c_n = ' + str(psi.coeff))

p.plotWaveFunction3d(psi, samples = 100, frames = 3600, timeFactor = 1.33, saveName = None)
